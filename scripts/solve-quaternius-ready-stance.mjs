...
const rp=process.argv[2]||'public/lab3d/motion/quaternius-rig-analysis.json',sp=process.argv[3]||'public/lab3d/motion/quaternius-ready-stance.json',out=process.argv[4]||'public/lab3d/motion/quaternius-ready-solve.json';
const r=JSON.parse(fs.readFileSync(rp,'utf8')),s=JSON.parse(fs.readFileSync(sp,'utf8')),J=Object.fromEntries(r.joints.filter(j=>!j.missing).map(j=>[j.name,j]));
...
const hipSpan=Math.abs(hipL.x-hipR.x),leftKneeFootDx=Math.abs(LL.joint.x-LL.end.x),rightKneeFootDx=Math.abs(LR.joint.x-LR.end.x);
const metrics={hipSpan,leftKneeFootDx,rightKneeFootDx,stanceWidth,kneeSpan,leftKneeLateralMargin:Math.abs(LL.joint.x-centerX),rightKneeLateralMargin:Math.abs(LR.joint.x-centerX),leftKneeAngleDeg:angle(hipL,LL.joint,LL.end),rightKneeAngleDeg:angle(hipR,LR.joint,LR.end),leftElbowAngleDeg:angle(shoulderL,AL.joint,AL.end),rightElbowAngleDeg:angle(shoulderR,AR.joint,AR.end),leftKneeClearance:LL.joint.y-floor,rightKneeClearance:LR.joint.y-floor,handSpan:dist(AL.end,AR.end),shoulderSpan:dist(shoulderL,shoulderR),leftFootError:dist(LL.end,s.target.leftFoot),rightFootError:dist(LR.end,s.target.rightFoot)};
const failures=[];if(!LL.reachable||!LR.reachable)failures.push('leg-unreachable');if(!AL.reachable||!AR.reachable)failures.push('arm-unreachable');if(metrics.leftKneeAngleDeg>165||metrics.rightKneeAngleDeg>165)failures.push('straight-knee');if(metrics.leftKneeClearance<.18||metrics.rightKneeClearance<.18)failures.push('knee-floor');if(metrics.handSpan>metrics.shoulderSpan*1.65)failures.push('t-pose');if(metrics.leftFootError>.025||metrics.rightFootError>.025)failures.push('foot-lock');if(!leftIpsilateral||!rightIpsilateral||!kneesSeparated)failures.push('knee-centerline-cross');if(metrics.kneeSpan<metrics.stanceWidth*.28)failures.push('knee-base-too-narrow');
if(metrics.leftKneeFootDx>metrics.stanceWidth*.38||metrics.rightKneeFootDx>metrics.stanceWidth*.38)failures.push('knee-foot-tracking');
if(metrics.leftKneeAngleDeg<85||metrics.leftKneeAngleDeg>155||metrics.rightKneeAngleDeg<85||metrics.rightKneeAngleDeg>155)failures.push('knee-flexion-envelope');
const frontalPlane={leftIpsilateral,rightIpsilateral,kneesSeparated};const report={frontalPlane,sourceRig:rp,sourceStance:sp,points,metrics,reachable:{leftLeg:LL.reachable,rightLeg:LR.reachable,leftArm:AL.reachable,rightArm:AR.reachable},gate:{pass:failures.length===0,failures}};fs.writeFileSync(out,JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(failures.length)process.exit(1);
