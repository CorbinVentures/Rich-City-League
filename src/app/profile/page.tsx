...
export default function ProfilePage() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    const roleResult = await supabase.from('profile_roles').upsert({ profile_id: user.id, role, status: role === 'fan' ? 'active' : 'pending' } as never);
    if (role === 'fan') await supabase.from('fan_profiles').upsert({ profile_id: user.id } as never);
    setSaved(!profileResult.error && !roleResult.error);
  };
