...
export default function ProfilePage() {
  ...
  return <main className="min-h-screen bg-rcl-black pb-24 text-white"><Container maxWidth="xl" className="py-10 sm:py-16">
    <div className="mx-auto max-w-5xl"><div className="max-w-3xl"><p className="text-xs font-black tracking-[.28em] text-rcl-gold">RCL IDENTITY CENTER</p><h1 className="mt-3 font-display text-4xl font-black uppercase sm:text-6xl">Create your <span className="text-rcl-orange">RCL profile.</span></h1><p className="mt-4 text-base leading-7 text-white/60">Choose how you participate in the Rich City League. Your role determines which parts of the platform are available to you.</p></div>
    {user ? <form onSubmit={save} className="mt-10 grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      <section className="rounded-3xl border border-white/10 bg-white/[.035] p-5 shadow-2xl sm:p-7"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black tracking-[.2em] text-white/40">01 · PARTICIPATION TYPE</p><h2 className="mt-2 font-display text-2xl font-black uppercase">Choose your lane</h2></div><span className="rounded-full border border-white/10 px-3 py-1 text-[9px] font-black tracking-widest text-white/40">RCL ID</span></div><div className="mt-6 grid gap-3 md:grid-cols-3">{roleCards.map(({ id, icon: Icon, title, subtitle, detail, accent }) => <button type="button" key={id} onClick={() => setRole(id)} className={`rounded-2xl border p-4 text-left transition ${role === id ? accent : 'border-white/10 bg-black/20 hover:border-white/25'}`}><div className="flex items-center justify-between"><Icon className={role === id ? 'text-rcl-orange' : 'text-white/40'} /><span className={`h-2 w-2 rounded-full ${role === id ? 'bg-rcl-orange' : 'bg-white/10'}`} /></div><p className="mt-5 font-display text-lg font-black">{title}</p><p className="mt-1 text-xs font-semibold text-white/70">{subtitle}</p><p className="mt-3 text-[11px] leading-5 text-white/45">{detail}</p></button>)}</div>
      {isAdmin && <div className="mt-4 flex gap-3 rounded-2xl border border-rcl-gold/30 bg-rcl-gold/10 p-4 text-sm"><FaShieldHalved className="mt-1 text-rcl-gold" /><p><strong>ADMIN ACCESS</strong><br /><span className="text-white/60">Your administrator permissions are controlled by league authorization and cannot be granted from this form.</span></p></div>}
      <div className="mt-8 grid gap-5 sm:grid-cols-2"><label className="block text-xs font-black uppercase tracking-widest text-white/45">Display name<input value={displayName} onChange={e => setDisplayName(e.target.value)} maxLength={80} className="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white outline-none focus:border-rcl-orange" /></label><label className="block text-xs font-black uppercase tracking-widest text-white/45">Username / role<br /><span className="mt-3 block rounded-xl border border-white/10 bg-white/[.02] px-4 py-3 text-sm text-white/55">{profile?.username ? `@${profile.username}` : role.toUpperCase()}</span></label></div>
      ...
      <aside className="space-y-4"><div className="rounded-3xl border border-white/10 bg-gradient-to-br from-rcl-navy/70 to-black p-6"><p className="text-[10px] font-black tracking-[.2em] text-rcl-gold">YOUR PROFILE PATH</p><div className="mt-6 space-y-4">{['Choose your identity','Build your profile','Get verified when required','Unlock your RCL experience'].map((item, index) => <div key={item} className="flex gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-[10px] font-black">0{index + 1}</span><p className="pt-1 text-sm font-semibold text-white/75">{item}</p></div>)}</div></div><div className="rounded-3xl border border-white/10 bg-white/[.025] p-6"><p className="text-[10px] font-black tracking-[.2em] text-white/35">SECURITY</p><p className="mt-3 text-sm leading-6 text-white/55">Need to change your password? Use the secure recovery flow instead of editing profile details.</p><a href="/auth/forgot-password" className="mt-4 inline-flex rounded-xl border border-white/10 px-4 py-2 text-xs font-black uppercase tracking-widest text-rcl-gold hover:border-rcl-gold/40">Reset password</a></div></aside>
    </form> : <div className="mt-10 rounded-3xl border border-white/10 bg-white/[.03] p-10 text-center"><p className="text-white/50">Sign in to create your RCL profile.</p><a href="/auth/sign-in" className="mt-5 inline-flex rounded-xl bg-rcl-orange px-6 py-3 font-black uppercase text-black">Sign in</a></div>}</div>
  </Container></main>;
}
