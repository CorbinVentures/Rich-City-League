...
export default function FriendsPage() {
  ...
  return <main className="min-h-screen bg-rcl-black pb-24 text-white"><Container maxWidth="lg" className="py-12">
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-6"><h2 className="text-xl font-bold">Find your people</h2><div className="mt-4 flex gap-2"><input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') void findPeople(); }} placeholder="Search any RCL member" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black px-4 py-3 text-sm outline-none focus:border-rcl-gold" /><button onClick={() => void findPeople()} className="rounded-xl bg-rcl-gold px-4 font-bold text-black">SEARCH</button></div>{message && <p className="mt-3 text-sm text-rcl-gold">{message}</p>}<div className="mt-5 space-y-3">{people.map(person => <div key={person.id} className="flex items-center justify-between rounded-xl bg-black/50 p-3"><SocialIdentity author={person} /><button onClick={() => void request(person.id)} className="text-xs font-bold text-rcl-gold">ADD FRIEND</button></div>)}</div></section>
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-6"><h2 className="text-xl font-bold">Your connections</h2><div className="mt-5 space-y-3">{friends.length === 0 && <p className="text-sm text-gray-500">Search for a teammate to get connected.</p>}{friends.map(row => <div key={row.id} className="flex items-center justify-between rounded-xl bg-black/50 p-3"><div><SocialIdentity author={connectionProfile(row)} /><p className="mt-1 text-xs uppercase tracking-widest text-gray-500">{row.status}</p></div>{row.status === 'pending' && row.addressee_id === user?.id ? <div className="flex gap-2"><button onClick={() => void update(row.id, 'accepted')} className="text-xs font-bold text-rcl-gold">ACCEPT</button><button onClick={() => void update(row.id, 'declined')} className="text-xs text-gray-400">DECLINE</button></div> : row.status === 'pending' && <button onClick={() => void update(row.id, 'cancelled')} className="text-xs text-gray-400">CANCEL</button>}</div>)}</div></section>
    </div>
  </Container></main>;
}
