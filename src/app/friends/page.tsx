...
export default function FriendsPage() {
  ...
  const findPeople = async () => {
    ...
    const pattern = `%${term}%`;
    const { data, error } = await supabase.from('profiles').select('id,display_name,username').eq('is_active', true).eq('profile_visibility', 'public').or(`display_name.ilike.${pattern},username.ilike.${pattern},first_name.ilike.${pattern},last_name.ilike.${pattern}`).neq('id', user?.id ?? '').limit(20);
    setPeople(data ?? []);
