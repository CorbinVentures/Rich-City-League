    ...
    <section className="relative overflow-hidden border-b border-white/10">
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-8 pt-12 sm:pt-16">
        ...
        <div className="mt-8 flex min-h-[250px] flex-col justify-end gap-6 sm:flex-row sm:items-end">
          ...
          <div className="flex-1"><div className="mb-3 flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full" style={{backgroundColor:accent,boxShadow:`0 0 16px ${theme.glow}`}}/><span className="text-[9px] font-black uppercase tracking-[.25em] text-white/40">Official franchise site</span></div><h1 className="font-display text-4xl font-black uppercase leading-none drop-shadow-2xl sm:text-6xl">{team.name}</h1><p className="mt-3 text-xs font-black uppercase tracking-[.2em] text-white/55">{team.city??'Richmond'}, Virginia · {division?.name??'Rich City League'}</p>{team.description&&<p className="mt-4 max-w-2xl text-sm leading-6 text-white/65">{team.description}</p>}</div>
          <div className="flex gap-3"><div className="min-w-28 border border-white/10 bg-black/45 px-5 py-4 text-center backdrop-blur-md" style={{boxShadow:`inset 0 1px 0 ${accent}44`}}><p className="text-[9px] uppercase tracking-widest text-white/40">Record</p><p className="mt-1 font-display text-3xl font-black">{record?`${record.wins}–${record.losses}`:'0–0'}</p></div></div>
        </div>
      </div>
    </section>
