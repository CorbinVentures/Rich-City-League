...
import { FaArrowRight, FaBasketball, FaBolt, FaBookmark, FaCheck, FaComment, FaCompass, FaFire, FaHeart, FaImage, FaMagnifyingGlass, FaPeopleGroup, FaPlay, FaPlus, FaShareNodes, FaTrash, FaUserGroup, FaVideo, FaXmark, FaBars, FaHouse, FaGear } from 'react-icons/fa6';
...
export default function SocialPage() {
  ...
  return <><SocialCinematicIntro /><main className="rcl-social-page relative min-h-screen bg-[#05080d] text-white pb-20 lg:pb-0"><ContentAssetBackground assetKey="social.cover" opacity={0.08} /><div className="relative z-10">
    <header className="rcl-social-header sticky top-0 z-40 border-b border-white/10 bg-[#05080d]/95 backdrop-blur-xl"><Container maxWidth="xl" className="flex h-16 items-center gap-4"><Link href="/" className="hidden items-center gap-2 sm:flex"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-rcl-orange text-black"><FaBasketball /></span><span className="font-display text-lg font-black uppercase tracking-tight">RCL <span className="text-rcl-orange">Social</span></span></Link><div className="relative min-w-0 flex-1 md:max-w-xl"><FaMagnifyingGlass className="absolute left-4 top-1/2 -translate-y-1/2 text-white/25" /><input value={search} onChange={(e) => { setSearch(e.target.value); setTab('feed'); }} placeholder="Search players, teams, runs…" className="w-full rounded-2xl border border-white/10 bg-white/[.04] py-2.5 pl-10 pr-4 text-sm outline-none transition focus:border-rcl-orange/50" /></div>{user ? <button onClick={openComposer} className="hidden rounded-xl bg-rcl-orange px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-black sm:inline-flex sm:items-center sm:gap-2"><FaPlus /> Create</button> : <Link href="/auth/sign-in?redirect=/social" className="rounded-xl border border-white/10 px-4 py-2 text-[10px] font-black uppercase tracking-widest">Sign in</Link>}<button type="button" onClick={() => setPlatformMenuOpen(true)} aria-label="Open Rich City League menu" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-rcl-orange/40 bg-black/40 text-rcl-orange transition hover:bg-rcl-orange hover:text-black"><FaBars /></button></Container></header>
    ...
    <div className="rcl-social-tabs fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 bg-[#05080d]/96 px-2 py-2 backdrop-blur-xl lg:hidden pb-[env(safe-area-inset-bottom)]">
      <div className="mx-auto flex max-w-xl gap-2 overflow-x-auto scrollbar-hide">
        {tabs.slice(0, 5).map((item) =>
          item.id === 'runs' ? (
            <Link key={item.id} href="/runs" className="flex shrink-0 items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white/45">
              {item.icon}<span>{item.label}</span>
            </Link>
          ) : item.id === 'communities' ? (
            <Link key={item.id} href="/communities" className="flex shrink-0 items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white/45">
              {item.icon}<span>{item.label}</span>
            </Link>
          ) : (
            <button key={item.id} onClick={() => selectTab(item.id)} className={`flex shrink-0 items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-wider ${tab === item.id ? 'border-rcl-orange bg-rcl-orange text-black' : 'text-white/45'}`}>
              {item.icon}<span>{item.label}</span>
            </button>
          )
        )}
    ...
    {error && <div className="fixed bottom-20 left-1/2 z-[60] max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-xl border border-white/10 bg-[#101722] px-4 py-3 text-xs text-white shadow-xl">{error}</div>}
  </div></main></>;
}
