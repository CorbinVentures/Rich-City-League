...
export default async function SocialPublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: posts }, { count: followers }, { count: following }, { data: repLevel }, { data: repLedger }, { data: earnedRepBadges }] = await Promise.all([
  ...
  const { data: authors } = authorIds.length ? await client.from('profiles').select('id,display_name,username,avatar_url,is_vip,vip_label').in('id', authorIds) as any : { data: [] };
