...
export default async function SocialPublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: posts }, { count: followers }, { count: following }] = await Promise.all([
    client.from('posts').select('id,body,media_urls,created_at').eq('author_id', id).order('created_at', { ascending: false }).limit(20) as any,
    ...
    <div className="mx-auto max-w-3xl">
      <section className="relative border-b border-white/10 bg-[#09111a]">
        ...
        <div className="px-5 pb-6">
          ...
          {profile.bio && <p className="mt-4 text-sm leading-6 text-white/65">{profile.bio}</p>}
      ...
      <section className="px-4 py-5"><div className="mb-4 flex items-center gap-2"><FaBasketball className="text-rcl-orange"/><h2 className="font-display text-xl font-black uppercase">Timeline</h2></div>
        {posts?.length ? <div className="space-y-4">{posts.map((post:any)=><article key={post.id} className="overflow-hidden rounded-3xl border border-white/10 bg-white/[.03]"><div className="p-5"><p className="whitespace-pre-wrap text-sm leading-6 text-white/75">{post.body}</p><p className="mt-3 text-[9px] text-white/25">{new Date(post.created_at).toLocaleDateString()}</p></div>{post.media_urls?.[0] && <div className="border-t border-white/10 bg-black"><img src={post.media_urls[0]} alt="Post media" className="max-h-[520px] w-full object-cover"/></div>}</article>)}</div> : <div className="rounded-3xl border border-dashed border-white/10 p-10 text-center text-sm text-white/35">No timeline posts yet.</div>}
      </section>
    </div>
