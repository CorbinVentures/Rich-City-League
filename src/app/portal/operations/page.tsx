...
export default function OperationsPage() {
  ...
  async function updateStatus(id: string, nextStatus: Registration['status']) {
    if (!client) return;
    const { error: updateError } = await client.from('registrations').update({ status: nextStatus, reviewed_at: new Date().toISOString() } as never).eq('id', id);
  ...
  async function updateGame() {
    ...
    const { error: updateError } = await client.from('games').update({
      ...
      status: score.status,
    } as never).eq('id', selectedGame);
    if (updateError) setError(updateError.message);
    else setGames((items) => items.map((item) => item.id === selectedGame ? { ...item, home_score: score.home, away_score: score.away, status: score.status } : item));
  }
  ...
  return <main><Container maxWidth="xl" className="py-12">
    ...
    <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-6"><h2 className="font-display text-2xl font-bold">Game management</h2><p className="mt-2 text-sm text-gray-400">Update scores and game status using the existing game record.</p><div className="mt-5 grid gap-3 sm:grid-cols-[2fr_1fr_1fr_1fr_auto]"><select aria-label="Select game" value={selectedGame} onChange={(event) => { const game = games.find((item) => item.id === event.target.value); setSelectedGame(event.target.value); if (game) setScore({ home: game.home_score, away: game.away_score, status: game.status }); }} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-white"><option value="">Select a game</option>{games.map((game) => <option key={game.id} value={game.id}>{new Date(game.scheduled_at).toLocaleString()} · {game.id.slice(0, 8)}</option>)}</select><input aria-label="Home score" type="number" min="0" value={score.home} onChange={(event) => setScore({ ...score, home: Number(event.target.value) })} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-white" /><input aria-label="Away score" type="number" min="0" value={score.away} onChange={(event) => setScore({ ...score, away: Number(event.target.value) })} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-white" /><select aria-label="Game status" value={score.status} onChange={(event) => setScore({ ...score, status: event.target.value as Game['status'] })} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-white">{(['scheduled', 'live', 'completed', 'cancelled', 'postponed'] as const).map((value) => <option key={value} value={value}>{value}</option>)}</select><button type="button" disabled={!selectedGame} onClick={() => void updateGame()} className="rounded-lg bg-rcl-gold px-4 py-2 font-semibold text-black disabled:opacity-50">Save</button></div></section>
