...
export default function NotificationsPage() {
  ...
  useEffect(() => { if (!supabase || !user) return; const load = async () => { const { data } = await supabase.from('notifications').select('*').eq('recipient_id', user.id).order('created_at', { ascending: false }).limit(100); setItems(data ?? []); }; void load(); }, [supabase, user]);
