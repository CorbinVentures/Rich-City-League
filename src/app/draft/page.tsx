...
export default function DraftNightPage() {
  ...
  return (
    <main className="rcl-draft-platform min-h-screen overflow-hidden pb-24">
      ...
      <Container maxWidth="xl" className="relative z-10 -mt-7">
        ...
        <section className="rcl-draft-workspace mt-8">
          ...
          <div className="rcl-draft-module rcl-player-preview">
            ...
            {selectedPlayer ? <><div className="rcl-player-photo">{selectedPlayer.photo_url ? <img src={selectedPlayer.photo_url} alt="" /> : <span>{selectedPlayer.first_name?.[0]}</span>}</div><h3>{playerName(selectedPlayer)}</h3><p>{selectedPlayer.position ?? 'Position TBD'} · OVR {ratings.get(selectedPlayer.id) ?? '—'}</p><div className="rcl-player-metrics"><span><b>{selectedPlayer.height_inches ? `${Math.floor(selectedPlayer.height_inches / 12)}'${selectedPlayer.height_inches % 12}"` : '—'}</b>HEIGHT</span><span><b>{selectedPlayer.weight_lbs ?? '—'}</b>WEIGHT</span><span><b>{selectedPlayer.hometown ?? 'RVA'}</b>HOME</span></div><Link href={`/players/${selectedPlayer.id}`} className="rcl-draft-secondary w-full">VIEW FULL PROFILE</Link></> : <div className="rcl-preview-empty"><FaShieldHalved /><strong>Select a player</strong><span>Scouting details appear here.</span></div>}
          </div>
