...
export default function ConversationPage() {
  ...
  const markRead = useCallback(async () => {
    ...
    await supabase.from('conversation_members').update({ last_read_at: new Date().toISOString() }).eq('conversation_id', conversationId).eq('profile_id', user.id);
  ...
  const send = async (event: React.FormEvent) => {
    ...
    setSending(true);
    const result = await supabase.from('messages').insert({ conversation_id: conversationId, sender_id: user.id, body: trimmed }).select('id,conversation_id,sender_id,body,created_at,deleted_at').single();
    if (result.error) setError(true);
    else {
      setMessages((current) => current.some((message) => message.id === result.data.id) ? current : [...current, result.data as Message]);
      setBody('');
      window.setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: 'smooth' }), 0);
    }
    setSending(false);
