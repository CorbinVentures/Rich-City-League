...
export default function ConversationPage() {
  ...
  const send = async (event: React.FormEvent) => { event.preventDefault(); if (!supabase || !user || !body.trim()) return; const { error } = await supabase.from('messages').insert({ conversation_id: params.conversationId, sender_id: user.id, body: body.trim() }); if (!error) setBody(''); };
