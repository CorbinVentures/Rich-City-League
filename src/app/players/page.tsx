...
export default async function PlayersPage() {
  ...
  const [{ data: players, error }, { data: seasons }, { data: teams }, { data: rosters }] = await Promise.all([
  ...
  const teamSeasonResult = activeSeasonId ? await client.from('team_seasons').select('id,team_id').eq('season_id', activeSeasonId) : { data: [] };
  const teamBySeason = new Map((teamSeasonResult.data ?? []).map(row => [row.id, row.team_id]));
