...
export default function ProductPage() {
  ...
  if (!product && notFound) return <main className="min-h-screen bg-rcl-black pb-24 text-white"><Container maxWidth="lg" className="py-20 text-center"><p className="rcl-kicker">RCL SHOP</p><h1 className="mt-3 font-display text-4xl uppercase">Product not found</h1><p className="mt-3 text-sm text-gray-500">That product is no longer available or the link is invalid.</p><Link href="/shop" className="rcl-button mt-6 inline-flex">Back to shop <FaArrowLeft /></Link></Container></main>;
  ...
  const favorite = async () => {
    if (!supabase || !user) { setMessage('Sign in to save gear.'); return; }
    const { error } = await supabase.from('product_favorites').upsert({ product_id: product.id, user_id: user.id } as never);
    setMessage(error ? 'Unable to save this product.' : 'Saved to your gear.');
  };
  const add = () => {
