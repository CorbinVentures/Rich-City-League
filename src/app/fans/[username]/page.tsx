...
export default async function FanProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  if (!client) notFound();
  const { data: profile } = await client.from('profiles').select('id,username,display_name,avatar_url,bio,location,created_at').eq('username', username).eq('is_active', true).maybeSingle() as any;
  if (!profile) notFound();
  const [{ data: fan }, { data: score }] = await Promise.all([
    client.from('fan_profiles').select('fan_level,games_attended,favorite_team_id').eq('profile_id', profile.id).maybeSingle() as any,
    client.from('score_explanations').select('score,components,data_points,calculated_at').eq('profile_id', profile.id).eq('score_type', 'fan_win_factor').order('calculated_at', { ascending: false }).limit(1).maybeSingle() as any,
  ]);
