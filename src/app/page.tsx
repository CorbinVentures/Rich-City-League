    ...
    <main className="rcl-future-site min-h-screen overflow-hidden pb-24 text-white lg:pb-0">
      ...
      <section className="rcl-future-hero">
        ...
        <Container maxWidth="xl" className="relative z-10 flex min-h-[40rem] items-end py-12 sm:py-16">
          <div className="w-full max-w-4xl">
            ...
            <h1 className="rcl-fx-title mt-6">RICH CITY<br />BUILDS <span className="accent">DIFFERENT.</span></h1>
            <p className="rcl-fx-subtitle mt-7">PLAY. COMPETE. CONNECT. GROW.</p>
      ...
      <Container maxWidth="xl" className="relative z-10 -mt-7 space-y-12 sm:-mt-10 sm:space-y-16">
        <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
          <div className="rcl-fx-card rcl-fx-live p-5 sm:p-6">
            ...
            {featuredGame ? (
              <Link href={'/games/' + featuredGame.id} className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-center">
                <div><p className="font-display text-lg font-black uppercase sm:text-2xl">{teamName(featuredGame.away_team_id)}</p><p className="mt-1 text-[9px] font-black tracking-[.16em] text-white/35">AWAY</p></div>
                <div>
                  {liveGame ? <><span className="rcl-fx-score">{featuredGame.away_score}</span><span className="mx-2 text-white/25">—</span><span className="rcl-fx-score">{featuredGame.home_score}</span></> : <span className="rcl-fx-score text-3xl">VS</span>}
                </div>
                <div><p className="font-display text-lg font-black uppercase sm:text-2xl">{teamName(featuredGame.home_team_id)}</p><p className="mt-1 text-[9px] font-black tracking-[.16em] text-white/35">HOME</p></div>
          ...
          <div className="rcl-fx-card p-5 sm:p-6">
            ...
            <div className="mt-4 divide-y divide-white/5">
              {nextGames.map((game) => <Link href={'/games/' + game.id} key={game.id} className="grid grid-cols-[3rem_1fr_auto] gap-3 py-3 hover:text-white"><div><strong className="block text-xs font-black">{formatDate(game.scheduled_at).split(',')[0]}</strong><small className="text-[9px] text-white/30">{formatTime(game.scheduled_at)}</small></div><div className="min-w-0"><p className="truncate text-[10px] font-black uppercase">{teamName(game.away_team_id)} <span className="px-1 text-white/25">VS</span> {teamName(game.home_team_id)}</p><small className="text-[9px] text-white/30">RICHMOND, VA</small></div><FaArrowRight className="mt-1 text-white/20" /></Link>)}
        ...
        <section className="grid gap-5 lg:grid-cols-[1.55fr_.75fr]">
          <div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <PlayerVisual rank="01" title="PLAYER DNA" subtitle="DISCOVER THE NEXT LEVEL" />
              <PlayerVisual rank="02" title="PLAYER IQ" subtitle="MEASURE YOUR IMPACT" />
              <PlayerVisual rank="03" title="RANKINGS" subtitle="SEE WHO IS RISING" />
            </div>
          </div>
        ...
        <section className="grid gap-5 lg:grid-cols-[1.4fr_.6fr]">
          <Link href="/lab" className="rcl-fx-lab-image group p-6 sm:p-8">
            <div className="relative z-10 max-w-lg">
              ...
              <ul className="mt-5 space-y-2 text-[10px] font-black tracking-[.12em] text-white/70"><li>✓ AI COACH</li><li>✓ PLAYER ANALYTICS</li><li>✓ GAME SIMULATIONS</li><li>✓ RANKING ENGINE</li><li>✓ FANTASY LEAGUES</li></ul>
        ...
        <section className="grid gap-5 lg:grid-cols-[1fr_.7fr]">
          <div className="rcl-fx-card p-5 sm:p-6"><SectionLabel title="LEAGUE PULSE" href="/standings" /><div className="mt-2">{snapshot.standings.slice(0, 5).map((standing, index) => <Link href="/standings" key={standing.id} className="rcl-fx-table-row hover:bg-white/[.02]"><span className="rcl-fx-rank">#{String(index + 1).padStart(2, '0')}</span><strong className="truncate text-xs font-black uppercase">{teamName(standing.team_id)}</strong><span className="text-[9px] font-black text-white/35">{standing.wins}W · {standing.losses}L</span></Link>)}</div></div>
          <div className="rcl-fx-card p-5 sm:p-6"><SectionLabel title="LATEST FROM RCL" href="/news" /><div className="mt-2 space-y-3">{snapshot.news.slice(0, 3).map((item) => <Link href={'/news/' + item.slug} key={item.id} className="block border-b border-white/5 pb-3 last:border-0"><p className="text-[9px] font-black tracking-[.15em] text-rcl-orange">RCL NEWS</p><h3 className="mt-1 text-xs font-black uppercase leading-5">{item.title}</h3></Link>)}{!snapshot.news.length && <p className="py-5 text-xs text-white/35">RCL stories will appear here.</p>}</div></div>
        </section>

        <footer className="border-t border-white/10 pt-8 pb-4"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="font-display text-3xl font-black italic tracking-tight">RCL</div><p className="mt-1 text-[8px] font-black tracking-[.25em] text-white/25">PLAYERS. PEOPLE. PURPOSE.</p></div><div className="text-[8px] font-black tracking-[.2em] text-white/25">RICHMOND, VA · BUILT BY THE COMMUNITY · FOR THE NEXT GENERATION.</div></div></footer>
      </Container>
    </main>
  );
}

function Quick({ href, icon, title, sub }: { href: string; icon: ReactNode; title: string; sub: string }) {
...
function PlayerVisual({ rank, title, subtitle }: { rank: string; title: string; subtitle: string }) {
  return <Link href="/players" className="rcl-fx-player-image group p-4"><span className="relative z-10 text-sm font-black tracking-[.15em] text-white/80">#{rank}</span><div className="relative z-10 mt-auto"><span className="text-[9px] font-black tracking-[.16em] text-rcl-orange">RCL SYSTEM</span><h3 className="mt-2 font-display text-2xl font-black uppercase">{title}</h3><p className="mt-1 text-[9px] font-black tracking-[.12em] text-white/45">{subtitle}</p></div></Link>;
}
