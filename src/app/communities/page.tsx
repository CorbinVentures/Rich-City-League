...
export default function CommunitiesPage() {
  ...
  const create = async (event: React.FormEvent) => { event.preventDefault(); if (!supabase || !user || !name.trim()) return; const slug = `${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${Date.now()}`; const { data } = await supabase.from('communities').insert({ name: name.trim(), slug, description: description.trim() || null, created_by: user.id, privacy: 'public', community_type: 'general' }).select().single(); if (data) { await supabase.from('community_members').insert({ community_id: data.id, profile_id: user.id, role: 'admin' }); setName(''); setDescription(''); await load(); } };
  const join = async (id: string) => { if (!supabase || !user) return; await supabase.from('community_members').insert({ community_id: id, profile_id: user.id }); };
