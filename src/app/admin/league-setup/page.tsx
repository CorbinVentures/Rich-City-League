...
export default function LeagueSetupPage() {
  ...
  async function submit(action:()=>Promise<{error:{message:string}|null}>) {
    if (!client) return;
  ...
  return <main className="min-h-screen bg-[#05080d] pb-20 text-white"><Container maxWidth="xl" className="py-10">
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      ...
      <form onSubmit={e=>{e.preventDefault();void submit(()=>client.from('seasons').insert({league_id:season.leagueId,name:season.name.trim(),slug:season.slug.trim().toLowerCase(),start_date:season.startDate,end_date:season.endDate,status:season.status} as never));}} className="rounded-3xl border border-white/10 bg-white/[.025] p-6"><p className="text-[9px] font-black uppercase tracking-[.22em] text-rcl-orange">02 · SEASON</p><h2 className="mt-2 font-display text-2xl font-black uppercase">Create season</h2><div className="mt-5 space-y-3"><select className={field} value={season.leagueId} onChange={e=>setSeason({...season,leagueId:e.target.value})}><option value="">Select league</option>{leagues.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><input className={field} value={season.name} onChange={e=>setSeason({...season,name:e.target.value})} placeholder="2026 Fall League"/><input className={field} value={season.slug} onChange={e=>setSeason({...season,slug:e.target.value.replace(/\\s+/g,'-')})} placeholder="2026-fall"/><div className="grid grid-cols-2 gap-3"><input type="date" className={field} value={season.startDate} onChange={e=>setSeason({...season,startDate:e.target.value})}/><input type="date" className={field} value={season.endDate} onChange={e=>setSeason({...season,endDate:e.target.value})}/></div><select className={field} value={season.status} onChange={e=>setSeason({...season,status:e.target.value as Season['status']})}>{(['draft','registration','active','completed','archived'] as const).map(x=><option key={x}>{x}</option>)}</select><button className={button} disabled={saving||!season.leagueId}>Create season</button></div></form>

      <form onSubmit={e=>{e.preventDefault();void submit(()=>client.from('divisions').insert({season_id:division.seasonId,name:division.name.trim(),age_group:division.ageGroup.trim()||null,gender:division.gender.trim()||null,max_teams:division.maxTeams?Number(division.maxTeams):null} as never));}} className="rounded-3xl border border-white/10 bg-white/[.025] p-6"><p className="text-[9px] font-black uppercase tracking-[.22em] text-rcl-orange">03 · DIVISION</p><h2 className="mt-2 font-display text-2xl font-black uppercase">Create division</h2><div className="mt-5 space-y-3"><select className={field} value={division.seasonId} onChange={e=>setDivision({...division,seasonId:e.target.value})}><option value="">Select season</option>{seasons.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><input className={field} value={division.name} onChange={e=>setDivision({...division,name:e.target.value})} placeholder="Open Division"/><div className="grid grid-cols-2 gap-3"><input className={field} value={division.ageGroup} onChange={e=>setDivision({...division,ageGroup:e.target.value})} placeholder="Adult"/><input className={field} value={division.gender} onChange={e=>setDivision({...division,gender:e.target.value})} placeholder="Open"/></div><input type="number" min="1" className={field} value={division.maxTeams} onChange={e=>setDivision({...division,maxTeams:e.target.value})} placeholder="Max teams (optional)"/><button className={button} disabled={saving||!division.seasonId}>Create division</button></div></form>
      ...
      <form onSubmit={e=>{e.preventDefault();void submit(()=>client.from('team_seasons').insert({team_id:teamSeason.teamId,season_id:teamSeason.seasonId,division_id:teamSeason.divisionId||null} as never));}} className="rounded-3xl border border-white/10 bg-white/[.025] p-6"><p className="text-[9px] font-black uppercase tracking-[.22em] text-rcl-orange">05 · TEAM SEASON</p><h2 className="mt-2 font-display text-2xl font-black uppercase">Assign team to season</h2><div className="mt-5 space-y-3"><select className={field} value={teamSeason.seasonId} onChange={e=>setTeamSeason({...teamSeason,seasonId:e.target.value,divisionId:''})}><option value="">Select season</option>{seasons.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select className={field} value={teamSeason.teamId} onChange={e=>setTeamSeason({...teamSeason,teamId:e.target.value})}><option value="">Select team</option>{teams.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select className={field} value={teamSeason.divisionId} onChange={e=>setTeamSeason({...teamSeason,divisionId:e.target.value})}><option value="">Division (optional)</option>{divisions.filter(x=>x.season_id===teamSeason.seasonId).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><button className={button} disabled={saving||!teamSeason.seasonId||!teamSeason.teamId}>Assign team</button></div></form>

      <form onSubmit={e=>{e.preventDefault();if(game.homeTeamId===game.awayTeamId)return;void submit(()=>client.from('games').insert({season_id:game.seasonId,division_id:game.divisionId||null,home_team_id:game.homeTeamId,away_team_id:game.awayTeamId,scheduled_at:new Date(game.scheduledAt).toISOString(),status:game.status,created_by:profile.id} as never));}} className="rounded-3xl border border-rcl-orange/20 bg-rcl-orange/[.03] p-6"><p className="text-[9px] font-black uppercase tracking-[.22em] text-rcl-orange">06 · GAME</p><h2 className="mt-2 font-display text-2xl font-black uppercase">Create scheduled game</h2><div className="mt-5 space-y-3"><select className={field} value={game.seasonId} onChange={e=>setGame({...game,seasonId:e.target.value,divisionId:'',homeTeamId:'',awayTeamId:''})}><option value="">Select season</option>{seasons.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select className={field} value={game.divisionId} onChange={e=>setGame({...game,divisionId:e.target.value})}><option value="">Division (optional)</option>{gameDivisions.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><div className="grid grid-cols-2 gap-3"><select className={field} value={game.homeTeamId} onChange={e=>setGame({...game,homeTeamId:e.target.value})}><option value="">Home team</option>{seasonTeams.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select className={field} value={game.awayTeamId} onChange={e=>setGame({...game,awayTeamId:e.target.value})}><option value="">Away team</option>{seasonTeams.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></div><input type="datetime-local" className={field} value={game.scheduledAt} onChange={e=>setGame({...game,scheduledAt:e.target.value})}/><button className={button} disabled={saving||!game.seasonId||!game.homeTeamId||!game.awayTeamId||!game.scheduledAt||game.homeTeamId===game.awayTeamId}>Create game</button><p className="text-xs leading-5 text-white/35">Create only from real league scheduling data. Demo records remain local seed data.</p></div></form>
    </div>
