  ...
  return <><LabCinematicIntro/><main className="labx">
    ...
    {screen==='drill'&&workout&&drill&&<section className="labx-shell drill"><div className="labx-crumb">THE LAB <i>›</i> WORKOUT <i>›</i> DRILL {activeDrill+1}<span>{activeDrill+1} OF {workout.drills.length}</span></div>
      ...
      <div className="labx-drill-body">
       ...
       {drillTab==='breakdown'&&<div className="labx-breakdown">{drill.coachingPoints.map((x,i)=><article key={x}><em>{i+1}</em><div className="labx-step-thumb"><FaPlay/></div><p><b>{x}</b><span>{i===0?'Identify the setup and spacing.':i===1?'Keep your options alive and read the reaction.':'Execute the skill under control.'}</span></p></article>)}</div>}
