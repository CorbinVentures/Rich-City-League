...
export function calculateWinFactor(input: { wins: number; games: number; impact: number[]; efficiency: number[]; defense: number[]; playmaking: number[]; consistency: number[] }): ExplainableScore {
  ...
  const components = [
    ...
    { label: 'Individual impact', value: average(input.impact), weight: 0.2, available: input.impact.length > 0 },
    { label: 'Efficiency', value: average(input.efficiency), weight: 0.2, available: input.efficiency.length > 0 },
    { label: 'Defense', value: average(input.defense), weight: 0.15, available: input.defense.length > 0 },
    { label: 'Playmaking', value: average(input.playmaking), weight: 0.1, available: input.playmaking.length > 0 },
    { label: 'Consistency', value: average(input.consistency), weight: 0.1, available: input.consistency.length > 0 },
  ];
  const available = components.filter((component) => component.available);
...
export function calculateFantasyPoints(stats: { points: number; rebounds: number; assists: number; steals: number; blocks: number; turnovers: number }, rules = { points: 1, rebounds: 1.2, assists: 1.5, steals: 3, blocks: 3, turnovers: -1 }) {
  return Number((stats.points * rules.points + stats.rebounds * rules.rebounds + stats.assists * rules.assists + stats.steals * rules.steals + stats.blocks * rules.blocks + stats.turnovers * rules.turnovers).toFixed(2));
}
