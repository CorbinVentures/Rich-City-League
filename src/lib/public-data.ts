...
export async function getLeagueSnapshot(): Promise<LeagueSnapshot> {
  const empty: LeagueSnapshot = { leagues: [], seasons: [], divisions: [], teams: [], teamSeasons: [], games: [], standings: [], news: [], venues: [] };
  ...
  const results = [leagues, seasons, divisions, teams, teamSeasons, games, standings, news, venues];
  const firstError = results.find((result) => result.error)?.error;
  if (firstError) console.error('Public league snapshot query failed; serving partial data', firstError);
...
export async function getTeamDetail(slug: string): Promise<TeamDetailData | null> {
  ...
  if (firstError) console.error('Public team detail query failed; serving partial data', firstError);
  ...
  if (seasonsResult.error || divisionsResult.error) console.error('Public team season query failed; serving partial data', seasonsResult.error ?? divisionsResult.error);
...
export async function getPlayerDetail(id: string): Promise<PlayerDetailData | null> {
  ...
  if (rosterResult.error || statsResult.error || iqResult.error || iqHistoryResult.error) {
    console.error('Public player detail query failed; serving partial data', rosterResult.error ?? statsResult.error ?? iqResult.error ?? iqHistoryResult.error);
  }
  ...
  if (teamSeasonResult.error) console.error('Public player team query failed; serving partial data', teamSeasonResult.error);
  const teamSeasons = teamSeasonResult.data ?? [];
  const teamIds = [...new Set(teamSeasons.map((item) => item.team_id))];
  const seasonIds = [...new Set(teamSeasons.map((item) => item.season_id))];
  const [teamsResult, seasonsResult, gamesResult] = await Promise.all([
    teamIds.length ? await client.from('teams').select('*').in('id', teamIds) : { data: [], error: null },
    seasonIds.length ? await client.from('seasons').select('*').in('id', seasonIds) : { data: [], error: null },
    statsResult.data?.length ? await client.from('games').select('*').in('id', statsResult.data.map((item) => item.game_id)) : { data: [], error: null },
  ]);
  const divisionsResult = seasonIds.length ? await client.from('divisions').select('*').in('season_id', seasonIds) : { data: [], error: null };
