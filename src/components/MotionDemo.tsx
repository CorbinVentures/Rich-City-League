...
const athletePhoto='https://images.unsplash.com/photo-1518063319789-7217e6706b04?auto=format&fit=crop&w=1600&q=90';
...
export function MotionDemo({title,skill,prescription,coachingPoints,commonMistakes}:Props){
 ...
 return <section className="mlab">
  <div className="mlab-crumb">THE LAB <i>›</i> SKILL LIBRARY <i>›</i> {skill.toUpperCase()} <i>›</i> <b>{title.toUpperCase()}</b></div><div className="mlab-title"><div><small>{skill.toUpperCase()}</small><h3>{title}</h3><p>Build a repeatable release from close range before adding distance.</p><div className="mlab-tags"><span>Touch</span><span>Mechanics</span><span>Release</span><span>Balance</span></div></div><b>{prescription}</b></div>
  <div className="mlab-screen">
   <img src={athletePhoto} alt="" className={'mlab-athlete '+view}/>
   <div className="mlab-shade"/><div className="mlab-quote"><b>DISCIPLINE<br/>CREATES<br/>FREEDOM.</b><span>“GREAT SHOOTERS<br/>AREN’T BORN.<br/>THEY’RE BUILT.”</span></div><div className="mlab-brand"><b>RCL</b><span>MOTION LAB</span></div>
   ...
   <div className="mlab-readout"><span>ARC <b>48°</b></span><span>RELEASE <b>0.42s</b></span><span>VIEW <b>{view.toUpperCase()}</b></span></div><div className="mlab-player"><button onClick={()=>setPlaying(v=>!v)}>{playing?<FaPause/>:<FaPlay/>}</button><span>{Math.round(progress*12/100).toString().padStart(2,'0')} / 12</span><i><b style={{width:progress+'%'}}/></i><FaVolumeHigh/><strong>1x</strong><FaExpand/></div>
  </div>
  ...
  <div className="mlab-details">
   ...
   <div><h5 className="orange">→ NEXT DRILL</h5><strong>Close Range Finishing</strong><p>Build touch and control around the rim after mastering your release.</p><button onClick={()=>{setProgress(0);setPlaying(true)}}>START NEXT →</button></div>
  </div>
 </section>
}