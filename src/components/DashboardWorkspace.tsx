...
export default function DashboardWorkspace() {
  ...
  return <main><Container maxWidth="xl" className="py-10 sm:py-14">
    ...
    {(role === 'player' || role === 'coach') && <div className="mt-6 grid gap-4 sm:grid-cols-3"><Link href="/stats" className="rounded-2xl border border-white/10 p-5 hover:border-rcl-gold/50">Statistics <span className="mt-1 block text-sm text-gray-400">View real game performance</span></Link><Link href="/register" className="rounded-2xl border border-white/10 p-5 hover:border-rcl-gold/50">Registration <span className="mt-1 block text-sm text-gray-400">Submit or review status</span></Link><Link href="/games" className="rounded-2xl border border-white/10 p-5 hover:border-rcl-gold/50">Live scoring <span className="mt-1 block text-sm text-gray-400">Follow live game status</span></Link></div>}
  </Container></main>;
}
