...
begin
  ...
  if d.clock_deadline_at is not null and now() >= d.clock_deadline_at then raise exception 'Pick expired; commissioner review required'; end if;
  select team_id into expected_team from public.draft_order where draft_id = target_draft and pick_number = d.current_pick;
  if expected_team is null then raise exception 'Draft order is not configured for the current pick'; end if;
  if not public.is_staff_or_admin() and not exists (select 1 from public.team_coaches tc where tc.team_id = target_team and tc.profile_id = auth.uid()) then raise exception 'You are not authorized to make this selection'; end if;
  if target_team <> expected_team then raise exception 'Your team is no longer on the clock'; end if;
  if not exists (select 1 from public.players p where p.id = target_player and p.is_active) then raise exception 'Player is not eligible for selection'; end if;
  if exists (select 1 from public.rosters r join public.team_seasons ts on ts.id = r.team_season_id where ts.season_id = d.season_id and r.player_id = target_player and r.left_at is null) then raise exception 'Player is already assigned to a roster'; end if;
  ...
  insert into public.draft_picks(draft_id, pick_number, round_number, team_id, player_id, selected_by)
    select target_draft, d.current_pick, round_number, target_team, target_player, auth.uid() from public.draft_order where draft_id = target_draft and pick_number = d.current_pick returning * into result;
  select ts.id into team_season from public.team_seasons ts where ts.season_id = d.season_id and ts.team_id = target_team;
  ...
  insert into public.roster_status_history(roster_id, status, reason, changed_by) values (roster_id, 'ACTIVE', 'Selected in RCL draft', auth.uid());
  insert into public.league_transactions(season_id, transaction_type, receiving_team_id, player_ids, status, reason, proposed_by, approved_by, executed_at) values (d.season_id, 'DRAFT', target_team, array[target_player], 'EXECUTED', 'RCL draft selection', auth.uid(), auth.uid(), now());
  select profile_id into player_profile from public.players where id = target_player;
  if player_profile is not null then insert into public.notifications(recipient_id, actor_id, type, title, body, link) values (player_profile, auth.uid(), 'draft_pick', 'You have been drafted!', 'Congratulations — you were selected in the RCL draft.', '/draft'); end if;
  ...
  insert into public.audit_logs(user_id, action, details) values (auth.uid(), 'PLAYER_DRAFTED', jsonb_build_object('draft_id', target_draft, 'team_id', target_team, 'player_id', target_player, 'pick_id', result.id)::text);
  return result;
end;
$$;
...
create or replace function public.manage_draft_clock(target_draft uuid, target_action text, target_extension_seconds integer default 0)
...
begin
  ...
  if target_action = 'OPEN' or target_action = 'RESUME' then
    ...
    update public.drafts set status = 'OPEN', clock_started_at = now(), clock_deadline_at = now() + make_interval(secs => remaining), clock_remaining_seconds = null where id = target_draft;
  elsif target_action = 'PAUSE' then
    remaining := greatest(0, ceil(extract(epoch from (coalesce(d.clock_deadline_at, now()) - now())))::integer);
    update public.drafts set status = 'PAUSED', clock_remaining_seconds = remaining, clock_started_at = null, clock_deadline_at = null where id = target_draft;
  elsif target_action = 'EXTEND' then
    if d.status <> 'OPEN' then raise exception 'Only an open draft clock can be extended'; end if;
    update public.drafts set clock_deadline_at = coalesce(clock_deadline_at, now()) + make_interval(secs => greatest(target_extension_seconds, 0)) where id = target_draft;
  elsif target_action = 'COMPLETE' then
    update public.drafts set status = 'COMPLETED', clock_started_at = null, clock_deadline_at = null where id = target_draft;
...
language plpgsql security definer set search_path = public
as $$
declare d public.drafts; team_count integer; ordered_team_id uuid;
begin
  ...
  select count(*) into team_count from public.team_seasons ts join public.teams t on t.id = ts.team_id where ts.season_id = d.season_id and t.is_active;
  if team_count = 0 or coalesce(array_length(ordered_teams, 1), 0) <> team_count * d.rounds then raise exception 'Draft order must contain one entry for every team in every round'; end if;
  for pick_index in 1..array_length(ordered_teams, 1) loop
    ordered_team_id := ordered_teams[pick_index];
    if not exists (select 1 from public.team_seasons ts join public.teams t on t.id = ts.team_id where ts.season_id = d.season_id and ts.team_id = ordered_team_id and t.is_active) then raise exception 'Draft order contains a team outside this season'; end if;
  end loop;
  delete from public.draft_order where draft_id = target_draft;
  for pick_index in 1..array_length(ordered_teams, 1) loop
    insert into public.draft_order(draft_id, pick_number, round_number, team_id) values (target_draft, pick_index, ((pick_index - 1) / team_count) + 1, ordered_teams[pick_index]);
  end loop;
end;
$$;
