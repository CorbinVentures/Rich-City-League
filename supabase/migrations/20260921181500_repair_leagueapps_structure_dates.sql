-- Repair the deployed LeagueApps structure materializer.
-- The prior migration file was edited after it had already run in production, so
-- production retained the original function body. Migrations are immutable once applied.
...
begin
  ...
  with raw as (
    ...
    select program_id,program_name,
      least((to_timestamp(start_ms/1000) at time zone 'UTC')::date,
            (to_timestamp(end_ms/1000) at time zone 'UTC')::date) start_date,
      greatest((to_timestamp(start_ms/1000) at time zone 'UTC')::date,
               (to_timestamp(end_ms/1000) at time zone 'UTC')::date) end_date
    from programs
  ), ins as (
    ...
    select target_league,program_name,
      ...
      start_date,end_date,'draft',program_id
    from normalized
  ...
  with raw as (
    ...
    on conflict (leagueapps_team_id) where leagueapps_team_id is not null do update set
  ...
  with raw as (
    ...
    on conflict (team_id,season_id) do update set
      division_id=coalesce(excluded.division_id,team_seasons.division_id)
    returning 1
